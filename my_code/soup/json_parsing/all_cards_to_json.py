...
def get_category_urls(base_url, schema):
    """Создаёт список ссылок на категории товаров."""
    category_urls = []
...
def get_page_card_urls(page_url, schema):
    """Парсит ссылки на товарные карточки текущей страницы."""
    page_card_urls = []
...
def get_all_card_urls(base_url, schema):
    """
    Парсит все ссылки на товарные карточки сайта
    и разбивает их по вложенным спискам согласно категории.
    """
    card_urls = []
    category_urls = get_category_urls(base_url, schema)
    for category_url in category_urls:
        card_urls.append(get_category_card_urls(category_url, schema))
    return card_urls
...
def parse_category_cards(category_url, schema, head_names):
    """Парсит все карточки товарной категории."""
    category_card_list = []
...
def parse_all_cards(base_url, schema, head_names):
    """Парсит все товарный карточки на сайте."""
    card_list = []
    category_urls = get_category_urls(base_url, schema)
    for category_url in category_urls:
        card_list.extend(
            parse_category_cards(category_url, schema, head_names)
        )
    return card_list
...
def main():
    ...
    try:
        ...
        pass
    file_path = os.path.join(dir_path, "all_cards.json")
    ...
    result_json = parse_all_cards(base_url, schema, head_names)
